...
//      型の代わりに「drinks が最大の選択肢＝その卓の正解」とみなして同じ物差しを当てる。
{
  ...
  if (lines.length) FAILURES.push(
    ...
    + `\n  → 文字数と報酬の対応を崩すこと（短い選択肢にも drinks:2 を配る）。`
...
//   4択なので当てずっぽうは25%。
//
//   要件は **50%以下**（作者判断・2026-08-09）。25%（＝完全に読めない）ではない理由：
//   残るテルの一部は、記号の癖ではなく「意味の型そのもの」から出ている。
//     凡打＝空ヨイショ  → 感嘆符が自然に出る（「！」を持つのは凡打53% / 正解1%）
//     ハズレ＝話題逸らし → 疑問形が自然に出る（末尾「？」はハズレ31% / 正解11%）
//   この2つを消すと、型が役割を演じられなくなって4択が痩せる。
//   「！」と末尾「？」の2特徴だけで38.5%当たるので、25%は原理的に狙えない。
//
//   到達までの道筋（実測・特徴を人為的に潰して測ったもの）:
//     現状                                        78.2%
//     長さ＋句読点＋引用符＋「──」＋「もん」を均す  53.4%
//       ＋数字も均す                              46.6%
//       ＋漢字率・敬語も均す（意味に近く難しい）    42.3%
//   つまり「長さと句読点と記号を均す」ところまでやれば50%に届く。
//   長さの寄与は7ポイントしかない（78.2→71.0）。主犯は句読点＝二文構造のほう。
//
...
//   数字を盛らないよう5分割交差検証で、学習に使っていない卓だけで採点する。
{
  ...
  console.log(`\n[表面だけの分類器] ${TABLES.length}卓・${D}特徴・5分割交差検証`
    + ` → 正解的中 ${acc.toFixed(1)}%（当てずっぽう25%／50%以下が要件）`);
  ...
  if (acc > 50) FAILURES.push(
    `本文の意味を一切見ない分類器が、表面の特徴だけで正解を ${acc.toFixed(1)}% 当てている（50%まで）。`
